...
public class CircularHoleGround : MonoBehaviour
{
    ...
    private MeshCollider meshCollider;
    private Mesh generatedMesh;
    private float lastHoleScale = -1f;

    void Start()
    {
        // Configurăm Rigidbody-ul să fie static pentru motorul de fizică
        Rigidbody rb = GetComponent<Rigidbody>();
        rb.isKinematic = true; 
        rb.useGravity = false;
        ...
        generatedMesh = new Mesh();
        generatedMesh.name = "GiantPhysicsGround";
        generatedMesh.MarkDynamic(); // Optimizează pentru modificări frecvente
    }
    ...
    void FixedUpdate()
    {
        if (hole != null)
        {
            // Pământul invizibil urmărește gaura
            transform.position = new Vector3(hole.position.x, transform.position.y, hole.position.z);

            // Verificăm dacă s-a schimbat dimensiunea gaurii
            if (hole.localScale.x != lastHoleScale)
            {
                lastHoleScale = hole.localScale.x;
                UpdateHolePhysicsMesh();
            }
        }
    }

    void UpdateHolePhysicsMesh()
    {
        float radius = lastHoleScale / 2f;
        ...
        // Aplicăm mesh-ul doar pe collider
        meshCollider.sharedMesh = generatedMesh;
    }
}